...
const outputTSDir = path.join(cwd, "app", "components", "icon");
...
const typesContent = await generateTypes({
  names: files.map((file) => {
    return JSON.stringify(file.replace(/\.svg$/, ""));
  }),
});

await writeIfChanged(path.join(outputTSDir, "names.ts"), typesContent);
