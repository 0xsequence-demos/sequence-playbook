...
function PlayCode({
  ...
  return (
    <div className="text-13 bg-white/10 sm:rounded-[12px] backdrop-blur-md overflow-hidden w-[calc(100%_+_2rem)] ml-[-1rem] md:ml-0 md:w-auto ">
      <div className="flex justify-between gap-4 px-6 pt-6">
        {copy ? <CopyExampleCode value={copy} /> : null}

        {steps.length > 1 ? (
          <button
            className="h-8 px-3 gap-2 rounded-[7px] bg-black/20 flex items-center justify-center "
            onClick={toggle}
          >
            <Icon
