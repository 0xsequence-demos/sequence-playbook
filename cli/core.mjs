...
export function createMagicMoveMachine(codeToKeyedTokens, options) {
  ...
  function commit(code, override) {
    ...
    ({ from: previous, to: current } = syncTokenKeys(
      previous,
      newTokens,
      mergedOptions,
    ));
...
export function toKeyedTokens(code, tokens, salt = "", lineNumbers = false) {
  ...
  const keyed = splitWhitespaceTokens(tokens)
    .flatMap((line, lineIdx) => {
      ...
      if (line.find((token) => token?.content.includes("**"))) {
        collapseIndex++;
        return false;
      }
      ...
      if (line.find((token) => token?.content.includes("$$"))) {
        const tokens = [
          {
            content: `${collapseIndex} lines collapsed`,
            htmlClass: "shiki-magic-move-collapsed",
          },
          { content: "\n", offset: lastOffset },
        ];
      ...
      // if (lineNumbers) {
      tokens.unshift({
        key: `${hash}-ln-${lineCount}`,
        content: `${String(lineCount).padStart(lineNumberDigits, " ")}  `,
        offset: firstOffset,
